...
csv_files = r"C:\Users\YR272YB\OneDrive - EY\Desktop\Projects\Utility_AHI\Original_Data\GlobalEYUtility_Final_for Demo.csv"
...
with col1:
    ...
    if col_op == 'Feeder_Switch_ID_V1':
        st.bar_chart(df.Feeder_Switch_ID_V1.value_counts())
    if col_op == 'Fee_Sw_Refe_ID':
        st.bar_chart(df.Fee_Sw_Refe_ID.value_counts())
...
with col2:
    ...
    if col_op == 'Max_TEMP':
        ...
        st.pyplot()
    if col_op == 'Customer At Risk':
        st.set_option('deprecation.showPyplotGlobalUse', False)
        num_plot(df, 'Customer At Risk')
        st.pyplot()
...
if st.checkbox("Check Outliers"):
    ...
    if nu_o == 'Max_TEMP':
        ...
        st.plotly_chart(fig)
    if nu_o == 'Customer At Risk':
        fig = pl.box(df, y="Customer At Risk")
        st.plotly_chart(fig)
