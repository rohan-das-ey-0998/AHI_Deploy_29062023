...
csv_files = r"C:\Users\YR272YB\OneDrive - EY\Desktop\Projects\Utility_AHI\raw_data\RawData_v1.csv"
...
if st.checkbox("Encoding Numerical variables"):
    ...
    if n_3 == 'Max_TEMP':
        ...
        fig.update_layout(title='Histogram', xaxis=dict(title='Values'), yaxis=dict(title='Count'))
        st.plotly_chart(fig)
    if n_3 == 'Customer At Risk':
        fig = go.Figure(data=[go.Histogram(x=df_featured['Customer At Risk'])])
        fig.update_layout(title='Histogram', xaxis=dict(title='Values'), yaxis=dict(title='Count'))
        st.plotly_chart(fig)
    ...
    a = ['No_of_times_exceeded_60_', 'LENGTH_OF_CABLE_MTRS_', 'NO__OF_JOINTS', 'AGE_YRS_',
         ...
         'Avg_TEMP', 'Max_TEMP','Customer At Risk']
...
try:
    df.to_csv(r"C:\Users\YR272YB\OneDrive - EY\Desktop\Projects\Utility_AHI\Original_Data\Preprocessed_data_v1.csv",index=False)
