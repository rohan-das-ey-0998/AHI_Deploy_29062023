...
#if st.checkbox("Quasi Constant i.e. same value for the largest part of dataset"):
#    vt_1 = VarianceThreshold(threshold=0.05)  ## i.e. variance=0
#    vt_1.fit(X_train_1)
#    st.write(X_train_1.columns[~vt_1.get_support()])

if st.checkbox("Duplicate Features"):
...
if st.checkbox("Correlated Features"):
    ...
    features = X_1
    target = y_1
    correlation_matrix = np.corrcoef(features.values.T, target)
    correlation_values = correlation_matrix[:-1, -1]
    d = dict(zip(features.columns,correlation_values))
    nu_p = st.selectbox("Select any Features to check the correlation value", ('Feeder_Category',
                                                                                'New_Summer',
                                                                                'New_Monsoon',
                                                                                'New_Winter',
                                                                                'Feeder',
    ...
    value = d.get(nu_p)
    st.write(f"Correlation value of {nu_p} with target is : {value}")


if st.checkbox("Embedding Methods"):
